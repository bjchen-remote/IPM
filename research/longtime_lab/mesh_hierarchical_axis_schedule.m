function [schedule,registration]=mesh_hierarchical_axis_schedule(search)
%MESH_HIERARCHICAL_AXIS_SCHEDULE Finite, state-independent integer refinement.
...
df=diff(f);dr=diff(r);assert(all(df==df(1))&&all(dr==dr(1)));
