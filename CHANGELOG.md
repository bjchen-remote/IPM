...
- `maxSteps=Inf` 现在是受支持的无步数上限配置；有限步数仍必须是正整数。
- 256 级第一象限长跑采用实际存储网格 `513×257`、`tau_final=1000`、每 `0.5` 个规范时间保存完整场切片、每 `25` 个规范时间尝试写 checkpoint。
...
- 核实旧 C 原始 H8 最长验签检查点是 `τ=14.2002012595`、`961×321`，未发现 `τ=15` 断点。该旧冻结场的 `1025×257`/`1281×321` 正式几何候选最大比为 `1.055349`/`1.042217`，X 核心为 `36.833`/`46.687`；新 C `τ=3.700010` 同类对照也倾向后者。`1281×321` 原始解析 `t=0` 无 LU 预检有合格首候选，注册 8 级，其中包含双向二倍。实际高分辨率 LU 与长跑尚未验证，参见 `research/longtime_lab/FOUR_TO_ONE_HIGH_RES_CHOICE_20260914.md`。

## 2026-09-14：R2.6 均摊密度层级搜索账本修复

- R2.5 从原始 t=0 到 canonical `τ=2.0649288466` 后，首次补充搜索在未保存的 step 1228 触发 `ipm:AutonomousMeshSearchEvidence`，安全回退并保存 step 1227 签名检查点。Y 核心由 `32.00179` 过线至 `31.998569` 时需要重网格；最后接受态 X/Y 最大相邻比 `1.0739662/1.0718093`、安全约 0.213，停止原因是 230 个实际第一阶段试探与登记 260 个不符，而非质量/资源门拒绝。
- 对密度版本 1/2 仅把补充阶段的圆滑参数上界扩至 44，第一阶段恢复 260 个新元组；初选 70 项、旧密度模式、原质量门、节点预算及 C 均不变。公开服务器接口测试新增实际搜索阶段 `70/260/170` 的逐项契约断言；同一签名态原 step 1228 重放规划成功，返回 42 个分级候选。
- 原 step 1227 检查点在独立目录用修复源码原生续算至 `τ=2.2`，step `1227→1323`，自主重布 `13→14`，正常到达设定终点；新检查点与旧停止轨道分开保存。完整证据及仍需连续长跑的边界见 `research/longtime_lab/BALANCED_SEARCH_STAGE_REPAIR_20260914.md`。
- 新事务原生审计通过：X/Y 核心 `41.026246/36.799648`、前沿 `24.185146`、相邻比 `1.068315/1.038745`、相对峰跳 `1.369e-7`、质量缺损 `2.861e-15`；末态 checkpoint 重新验签通过。完整 `ipm.verify('all','quick')`（94.7729 秒）及旧数值等价快速套件（5.7372 秒）均 exit0。
