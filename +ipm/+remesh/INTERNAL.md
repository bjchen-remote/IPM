...
运行调度层捕获第一象限 REMESH 异常、保留已接受网格并发布警告。
...
特征网格单元计数可以触发提案、参与候选设计/验收并产生重网格后质量告警；
...
`levelSetRetryDecision` 与 `recordLevelSetFailure` 保存失败时的核心中心及横/纵核心宽度；
只有几何变化达到一个目标单元才重新调用 `directLevelSetAdapt`。该门没有计步或计时
冷却，因此不会因等待固定步数错过快速形态变化，也不会在形态静止时重复构轴。
...
R2.5 未补齐此阶段，在首次触发补充搜索时产生账本异常；修复由
controller 回归测试保留。
