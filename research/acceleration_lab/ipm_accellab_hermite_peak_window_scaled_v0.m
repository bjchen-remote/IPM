function report = ipm_accellab_hermite_peak_window_scaled_v0(values,forcing,x,Dx,window,user)
%IPM_ACCELLAB_HERMITE_PEAK_WINDOW_SCALED_V0 Frozen pre-audit research helper.
...
[P,index] = max(samples);
span = diff(window); scale = max(1,max(abs(values)));
valueTolerance = opts.activeRelativeTolerance*scale;
positionTolerance = opts.positionRelativeTolerance*max([1,abs(window),span]);
eligible = boundary | abs(derivative)*span <= opts.stationarityRelativeTolerance*scale;
nearMaximum = samples >= P-valueTolerance;
active = find(nearMaximum & eligible);
[~,order] = sort(positions(active)); active = active(order);
representatives = [];
for i = active
    if isempty(representatives) || abs(positions(i)-positions(representatives(end))) > positionTolerance
...
if any(curvature(interiorActive)*span^2 >= -opts.minimumRelativeCurvature*max(P,realmin))
...
report = struct('kind','independent_linear_C1_hermite_maximum_functional', ...
    ...
    'activeValueTolerance',valueTolerance,'activePositionTolerance',positionTolerance, ...
