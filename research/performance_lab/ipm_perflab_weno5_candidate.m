function fluxDerivative = ipm_perflab_weno5_candidate( ...
    q,a,J,computationalSpacing,options,blockSize)
%IPM_PERFLAB_WENO5_CANDIDATE Isolated blocked-face performance candidate.
%   Research-only copy of ipm.field.weno5FluxDerivative, 2026-09-08.
%   The only arithmetic-path change batches independent face columns.
%   This is never called by ipm.solve. BLOCKSIZE defaults to 16.
...
for firstFace = 0:blockSize:numberOfNodes
    faces = firstFace:min(firstFace+blockSize-1,numberOfNodes);
    positiveIndices = faces(:)+(1:5);
    negativeIndices = faces(:)+(6:-1:2);
    positiveStencil = reshape(positiveFlux(:,positiveIndices), ...
        numberOfLines,numel(faces),5);
    negativeStencil = reshape(negativeFlux(:,negativeIndices), ...
        numberOfLines,numel(faces),5);
    faceFlux(:,faces+1) = weno_z_left( ...
