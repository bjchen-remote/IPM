function fluxDerivative = ipm_perflab_weno5_wall_ghosts( ...
...
% Research-only: retain the original full-line-count boundary GEMM results.
indices = [1:3,numberOfNodes+(4:6)];
extendedQ(:,indices) = options.wallQGhost;
extendedA(:,indices) = options.wallAGhost;
extendedJ(:,indices) = options.wallJGhost;
