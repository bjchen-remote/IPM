function [candidates,report]=mesh_plan_reference_node_count(feature,sourceAxes,referenceAxes,anchor,policy)
%MESH_PLAN_REFERENCE_NODE_COUNT Research-only reuse with actual old features.
% The field is never upsampled to manufacture a new source snapshot.
% Same-N or registered per-axis doubling only; not a native transaction API.
assert(isstruct(feature)&&isscalar(feature)&&all(isfield(feature, ...
    {'coreInterval','frontInterval','yCoreWidth','coreCenter','actualCoreCells'})));
validate_policy(policy);x=sourceAxes.x(:)';y=sourceAxes.y(:);
validate_reference(referenceAxes,x,y,anchor);
...
report=struct('kind','research_reference_node_count_axis_pairs_v2','policy',policy,'feature',feature, ...
    'nodeCount',[newNx,newNy],'actualSourceNodeCount',[numel(x),numel(y)],'xLimits',x([1,end]),'yLimits',y([1,end])', ...
    ...
    'noLU',true,'noTransfer',true,'noFlow',true,'sameBoxSameNodeCount',sameN,'nativeNodeCountContractSupported',sameN,'globalFeasibilityClaim',false);
...
function validate_reference(r,x,y,a)
...
assert(ismember(numel(rx),[numel(x),2*numel(x)-1])&& ...
    ismember(numel(ry),[numel(y),2*numel(y)-1]), ...
    'ipm:ResearchNodeLadder','Only the four predeclared per-axis doubling combinations are supported.');
assert(isequal(rx,-fliplr(rx))&&ry(1)==0&&isequal(x([1,end]),rx([1,end]))&&isequal(y([1,end]),ry([1,end])), ...
    'ipm:AutonomousMeshReference','The reference family must preserve actual units and physical domain endpoints.');
end
